...
class BasicStoryTest extends Specification {
    ...
    def "Test correct notifications for story level reporter"() {
        ...
        then:
        1 * notifier.fireTestStarted({it.displayName.startsWith("Story: basic_story")} as Description)
        then:
        1 * notifier.fireTestFinished({it.displayName.startsWith("Story: basic_story")} as Description)
    }
