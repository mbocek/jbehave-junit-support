...
import org.jbehavesupport.runner.story.NestedGivenStories
...
class NestedGivenStoryTest extends Specification {
    ...
    def "Test correct notifications"() {
        given:
        def runner = new JUnitRunner(NestedGivenStories)
        ...
        then:
        1 * notifier.fireTestStarted({it.displayName.equals("Story: NestedGivenStory")} as Description)
        then:
        ...
        then:
        1 * notifier.fireTestStarted({it.displayName.contains("GivenStoryWithNestedGivenStory")} as Description)
        then:
        1 * notifier.fireTestFinished({it.displayName.contains("GivenStoryWithNestedGivenStory")} as Description)
        then:
        ...
        then:
        1 * notifier.fireTestFinished({it.displayName.equals("Story: NestedGivenStory")} as Description)
        then:
    ...
    def "Test descriptions"() {
        ...
        then:
        desc.testClass == NestedGivenStories
        children.size() == 3
    ...
    def "Test correct notifications for story level reporter"() {
        ...
        1 * notifier.fireTestFinished({it.displayName.startsWith("BeforeStories")} as Description)
        then:
        1 * notifier.fireTestStarted({it.displayName.startsWith("Story: NestedGivenStory")} as Description)
        then:
        1 * notifier.fireTestFinished({it.displayName.startsWith("Story: NestedGivenStory")} as Description)
        then:
        1 * notifier.fireTestStarted({it.displayName.startsWith("AfterStories")} as Description)
        then:
        1 * notifier.fireTestFinished({it.displayName.startsWith("AfterStories")} as Description)
