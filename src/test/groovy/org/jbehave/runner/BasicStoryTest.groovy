...
class BasicStoryTest extends Specification {
    ...
    def "test"() {
        ...
        then:
        1 * notifier.fireTestStarted({((Description)it).displayName.equals("Story: BeforeStories")})
        then:
        1 * notifier.fireTestFinished({((Description)it).displayName.equals("Story: BeforeStories")})
        then:
        ...
        then:
        1 * notifier.fireTestStarted({((Description)it).displayName.equals("Story: AfterStories")})
        then:
        1 * notifier.fireTestFinished({((Description)it).displayName.equals("Story: AfterStories")})
    }
}
